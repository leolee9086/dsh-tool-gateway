...
export const DENY_REASON = '工具调用方式已改变：本会话只暴露 find_tools 与 call_tool。'
 ...
 *
 * @param {object} ctx 插件上下文
 * @param {Set<string>} keep 允许模型直接调用的工具名
 * @param {(agent: object|undefined) => boolean} enabledFor 这个 agent 的会话要不要施加约束
 * @returns {void}
 */
export function installGuard(ctx, keep, enabledFor) {
  ctx.effect(
    () => ctx.tools.guard((execution) => {
      const name = execution?.name
      if (typeof name === 'string' && keep.has(name)) return undefined
      // 带 parent 的是 call_tool 发起的子分发，不是模型直接调用。
 ...
 * @returns {void}
 */
export function installNotice(ctx, enabledFor) {
  ctx.effect(
    () => ctx.systemPrompt.section({
      name: 'tool-gateway',
      order: NOTICE_ORDER,
      text: (context) => (enabledFor(context?.agent) ? noticeText() : ''),
    }),
