 ...
 *
 * **会话必须活着。** 路由只服务"当前打开着的会话" —— chip 就长在那个会话的标题栏上，
 * 冷会话根本不会渲染它。所以查不到 live agent 就如实返回 404，不猜、不兜底。
 *
 * @module dsh-tool-gateway/route
 */

import { ownerSessionId, parentLookup } from './session-key.js'
import { createSwitchNotice } from './switch-notice.js'
 ...
 *
 * @param {object} ctx 插件上下文
 * @param {{isEnabled: Function, setEnabled: Function}} state 开关状态句柄
 * @returns {(req: object, res: object) => Promise<void>} handler
 */
export function createGatewayRoute(ctx, state) {
  return async function handle(req, res) {
    ...
    try {
      ...
      if (req.method === 'GET') {
        const { ownerId } = resolveOwner(ctx, sessionIdFromQuery(req.url))
        json(res, 200, { ok: true, enabled: state.isEnabled(ownerId) })
        return
      }
      if (req.method === 'POST') {
